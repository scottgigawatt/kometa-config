...
.PHONY: check check-generated format help lint lint-ci test-collections test-library test-seasonal test-subgenres test-tv-seasonal validate
...
#
# test-collections: Preview native movie and TV collections in isolated fixtures.
#
# Dependencies: Docker, TEST_ENV, and the two documented Plex test libraries.
#
...
#
# test-subgenres: Preview only the native ranked movie themes.
#
# Dependencies: Docker, TEST_ENV, and the documented movie test library.
#
test-subgenres:  # Preview rule-based subgenres without overlays or downloads.
	@scripts/run-collection-tests.sh --subgenres-only
