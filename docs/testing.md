...
The movie preview also loads all seven collections in `movies/genre.yml` and all 101 searches in `movies/subgenre-top.yml`. Horror, War, and Western follow Plex genre tags. LGBTQ+, Sports, Spy, and Stand-up Comedy combine movies from the named [TMDb keywords](https://kometa.wiki/en/latest/files/builders/tmdb/standard/keyword/) commented in source. These genres have no curated additions or external list synchronization. Genre browsing remains alphabetical and the custom posters remain local.

All 101 ranked themes live in `movies/subgenre-top.yml`, with custom posters, weekly schedules, hidden visibility, a 250-item library limit, and release-order browsing. Ninety use [TMDb Discover](https://kometa.wiki/en/latest/files/builders/tmdb/discover/movie/), ordered by rating when selecting up to 1,000 candidates with an original language of English. Pipe-separated keyword IDs mean OR; comma-separated genre IDs mean AND. Romantic Comedy requires both Romance and Comedy, while Romantic Drama requires Romance and Drama. Utopian excludes the dystopia keyword. Video Game uses the adaptation keyword, and Robots includes robots generally rather than only giant robots. The `ranked_theme` template builds each local poster path from its explicit `poster` slug and weekly schedule from its `day`; collection summaries and provider-specific rules stay beside each definition.

Eleven themes use native [IMDb keyword searches](https://kometa.wiki/en/latest/files/builders/imdb/search/): Chick-flick, Epics, Experimental, Historical Event, Medical, Melodrama, Mindfuck, Psychedelic, Spaghetti Western, Splatter, and Urban Fantasy. These searches cover themes with sparse TMDb tagging; Spaghetti Western also keeps IMDb's English-language matching for multilingual films. They require no IMDb account and never load personal lists. The normal rating/vote floors are 5 and 1,000; explicit per-theme overrides preserve broader selections, including Whodunit at 2/100, Bigfoot at 5/500, and Heartbreak at 1/10. Ratings and tags determine membership, so films can enter or leave as provider metadata changes.

The preview validates the entire theme file and its shared templates before connecting to Plex. Offline regression checks preserve all collection names, poster mappings, schedules, and rating/vote floors, and reject personal-list builders, external templates, download clients, unbounded searches, and undocumented IDs.

To preview only all 101 themes in `test_movie_lib`, including out-of-schedule collections:
...
The movie fixtures include a two-second black, silent clip matched to Bo Burnham: Make Happy (2016) for Stand-up Comedy. Tiny clips for Ferris Bueller's Day Off, RoboCop, Wonder Woman, Star Trek: The Motion Picture, Star Trek: Generations, Star Trek (2009), Predator, and AVP provide coverage for the city and universe rules. These disposable fixtures avoid copying production movies. Review all seven genre posters, 101 rule-based theme posters, six city posters, and five universe posters in `test_movie_lib`; a successful empty result is not an artwork test.
